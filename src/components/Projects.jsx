...
import postPal from '../images/post-pal.png';
...
const projectsData = [
  ...
  },
  {
    ...
    description: 'Developed a full-stack web application that generates AI-powered captions, relevant hashtags, and tips for social media platforms based on an uploaded image using Google Cloud Vision API and Google Gemini API.',
    demoLink: '#',
...
const ProjectCard = ({ project, isLeft }) => {
  return (
    <div className={`project-card ${isLeft ? 'left' : 'right'} relative group transform hover:scale-105 transition duration-300 bg-gray-800 rounded-lg overflow-hidden mb-6`}>
      ...
      <div className="p-4">
        ...
        <p className="text-lightGray mb-10">{project.description}</p>
        {project.id !== 4 && (
...
const Projects = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 font-mono">
      ...
      <div className={`flex ${isMobileView ? 'flex-col space-y-8' : 'md:grid md:grid-cols-2 md:gap-8'} px-4 md:px-8 lg:px-32`}>
        {projectsData.map((project, index) => (
          <div key={project.id} className="relative" style={{ marginTop: !isMobileView && index % 2 === 1 ? '12rem' : '0rem' }}>
            <ProjectCard project={project} isLeft={!isMobileView && index % 2 === 0} />
          </div>
        ))}
      </div>
    </div>
