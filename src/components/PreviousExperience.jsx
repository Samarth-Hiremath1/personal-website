...
//Below code has animation for the descriptions so that they appear after the logo appears. 
//But there is some error where the animation happens twive
...
//       const timer = setTimeout(() => {
//         setActiveIndex((prevIndex) => prevIndex + 1);
//       }, 2000); // Total time for all animations for one experience (2.5s + buffer)
...
//             <div className="w-1/3 text-right pr-6">
//               <h3 className={`text-2xl font-bold ${exp.color} ${index <= activeIndex ? 'animate-fadeInDelayed2' : ''}`} style={{ animationDelay: '1.5s' }}>{exp.company}</h3>
//               <p className={`text-lightGray italic ${index <= activeIndex ? 'animate-fadeInDelayed2' : ''}`} style={{ animationDelay: '1.5s' }}>{exp.dates}</p>
//             </div>
//             <div className="w-1/12 flex justify-center relative">
//               <div className={`h-36 w-36 bg-white p-1 rounded-full overflow-hidden z-10 ${index <= activeIndex ? 'animate-fadeIn' : ''}`}>
//                 <img src={exp.logo} alt={exp.company} className="h-full w-full object-contain" />
//               </div>
//               {index < experiences.length - 1 && (
//                 <div className={`absolute h-full w-1 bg-white top-full left-1/2 transform -translate-x-1/2 ${index <= activeIndex ? 'animate-drawLine' : ''}`} />
...
//             </div>
//             <div className="w-1/2 pl-6">
//               <h2 className={`text-lg text-white font-bold ${index <= activeIndex ? 'animate-fadeInDelayed2' : ''}`} style={{ animationDelay: '1.5s' }}>{exp.title}</h2>
//               <p className={`text-lightGray ${index <= activeIndex ? 'animate-fadeInDelayed2' : ''}`} style={{ animationDelay: '1.5s' }}>{exp.description}</p>
//             </div>
//           </div>
//         ))}
//       </div>
